...
let bets_to_string (bets : Bet.t list) : string =
  bets 
  |> List.map Bet.to_string  (* Assumes there is a [to_string] function in the Bet module *)
  |> String.concat ";"
...
let load_from_file (filename : string) : User.t =
  ...
  List.iter (fun bet -> User.add_bet user (Bet.match_of_bet bet) (Bet.team_of_bet bet) (Bet.amount_of_bet bet)) active_bets;
  
  (* Assuming the User module has a function to add a bet directly to the history, 
      you may need a different approach if this is not allowed *)
  List.iter (fun bet -> User.add_bet user (Bet.match_of_bet bet) (Bet.team_of_bet bet) (Bet.amount_of_bet bet)) bet_history;
  user

...
let make_match match_id a b odds =
  { id = match_id; a_side = a; b_side = b; match_odds = odds }

let to_string match_ =
  string_of_int match_.id ^ "/" ^ match_.a_side ^ "/" ^ match_.b_side
  ^ "/(Odds:" ^ match_.match_odds ^ ")"
...
let of_string str =
  try
    ...
    | [ id; a_side; b_side; odds_with_paren ] ->
        make_match (int_of_string id) a_side b_side odds_with_paren
